...
<template>
    <nav class="fixed bottom-0 left-0 w-full flex lg:hidden flex-row justify-between bg-white z-20 shadow-[0_-1px_3px_rgba(0,0,0,0.2)] border-t border-t-gray-300 px-4 py-2.5 dark:bg-gray-900 dark:shadow-[0_-1px_3px_rgba(0,0,0,0.5)] dark:border-t-black translate-y-0 duration-500"
        ...
        <ul class="flex flex-1 flex-row justify-around items-center text-center" role="menu">
            <li role="menuitem" v-if="lastScrollY">
                <a href="" @click.stop.prevent="scrollToTop"
                    ...
                    <span class="!flex" v-text="t('message.to_top')"></span>
                </a>
            </li>

            <li role="menuitem" v-if="installButtonVisibility" class="hidden xs:flex lg:hidden">
                <install-app-button v-model="installButtonVisibility" css-classes="!flex-col !mx-0" />
            </li>

            <li role="menuitem" v-if="showAddTask">
                <a href="/tasks/new/" class="navigation-button flex !flex-col !px-4">
